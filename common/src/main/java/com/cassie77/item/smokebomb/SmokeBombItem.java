...
public class SmokeBombItem extends Item implements ProjectileItem {
    ...
    public Projectile asProjectile(Level level, Position pos, ItemStack stack, Direction direction) {
        return new SmokeBombEntity(level, pos.x(), pos.y(), pos.z(), stack);
    }
    ...
    public InteractionResult use(Level level, Player user, InteractionHand hand) {
        ...
        if (level instanceof ServerLevel serverLevel) {
            Projectile.spawnProjectileFromRotation(SmokeBombEntity::new, serverLevel, itemStack, user, 0.0F, POWER, 1.0F);
        }
